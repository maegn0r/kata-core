...
class Post {
    ...
    public static class UntrustworthyMailWorker implements MailService {
        ...
        public Sendable processMail(Sendable mail) {
            Sendable handledMail;
            Sendable handledMailToSend = null;
            for (int i = 0; i < mailService.length; i++) {
                if (i == 0) {
                    handledMailToSend = mailService[i].processMail(mail);
                    continue;
                }
                handledMail = mailService[i].processMail(handledMailToSend); // maybe can optimize that
                handledMailToSend = handledMail;
            }
            return realMailService.processMail(handledMailToSend);
        }
    }
    ...
    public static class Spy implements MailService {
        ...
        public Sendable processMail(Sendable mail) {
            if (mail instanceof MailMessage) {
                if (mail.getTo().contains(AUSTIN_POWERS) || mail.getFrom().contains(AUSTIN_POWERS)) {
    ...
    public static class Thief implements MailService {
        ...
        public Sendable processMail(Sendable mail) {
            if (mail instanceof MailPackage) {
                if (((MailPackage) mail).getContent().getPrice() >= minValueToStole) {
                    stolenValue = ((MailPackage) mail).getContent().getPrice() + stolenValue;
                    return new MailPackage(mail.getFrom(), mail.getTo(), new Package(String.format("stones instead of %s", ((MailPackage) mail).getContent().getContent()), 0));
                }
            }
            return mail;
        }
        ...
        public Sendable processMail(Sendable mail) {
            if (mail instanceof MailPackage) {
                if (((MailPackage) mail).getContent().getContent().contains("stones")) {
                    throw new StolenPackageException("Warning! Stolen package!");
                }
                if (((MailPackage) mail).getContent().getContent().contains(WEAPONS) || (((MailPackage) mail).getContent().getContent().contains(BANNED_SUBSTANCE))) {
                    throw new IllegalPackageException("Warning! Forbidden package!");
                }
            }
            return mail;
        }
